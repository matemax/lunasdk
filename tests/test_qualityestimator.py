...
from tests.schemas import jsonValidator, QUALITY
...
class TestEstimateQuality(DetectTestClass):
    ...
    def setup_class(cls):
        ...
        cls.warp = [cls.warper.warp(detector.detectOne(VLImage.load(filename=ONE_FACE))) for detector in cls.detectors]
    ...
    def assertQualityReply(qualityDict: dict):
        ...
        """
        assert (
            jsonValidator(schema=QUALITY).validate(qualityDict) is None
        ), f"{qualityDict} does not match with schema {QUALITY}"
    ...
    def test_estimate_quality_(self):
        ...
        for idx, detector in enumerate(self.detectors):
            with self.subTest(detectorType=detector.detectorType):
                quality = self.qualityEstimator.estimate(self.warp[idx].warpedImage)
    ...
    def test_estimate_quality_as_dict(self):
        ...
        for idx, detector in enumerate(self.detectors):
            with self.subTest(detectorType=detector.detectorType):
                qualityDict = self.qualityEstimator.estimate(self.warp[idx].warpedImage).asDict()
